...
void push(int value) {
	...
	node_ptr->next_ptr = top_ptr; // следующий известен

	top_ptr = node_ptr; // теперь верхний ито этот

	if (counter == 0) { // то и нижний тоже
		buttom_ptr = node_ptr;
	}
...
int pop() {
	int rec = buttom_ptr->data; // результат ф-ции 
